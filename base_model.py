...
class BaseModel(object):
    def __init__(self, args, num_users, num_items):
        ...
        self.learner = args.learner  # adam, sgd
        self.init_stddev = args.init_stddev
        self.loss = args.loss  # 默认对数损失函数
        self.lambda_id_emb = args.reg_id_embedding  # 嵌入正则项
        self.lambda_others = args.reg_others  # 一般变量正则项
        self.eta = args.eta

    def build_train_model(self, model_vector, model_len, ratings, model_params):
        w_output = tf.Variable(
            tf.truncated_normal([model_len, 1], stddev=self.init_stddev, mean=0),
            name='w_output', dtype=tf.float32)

        b_output = tf.Variable(
            tf.truncated_normal([1], stddev=self.init_stddev * 0.01, mean=0),
            name='b_output', dtype=tf.float32)

        model_params.append(w_output)
        model_params.append(b_output)

        raw_predictions = tf.nn.xw_plus_b(
            model_vector, w_output, b_output, name='output')
        ...
        with tf.name_scope('error'):
            ...
            if type_of_loss == 'cross_entropy_loss':
                raw_error = tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=tf.reshape(raw_predictions, [-1]),
                    labels=tf.reshape(self.ratings, [-1])
                )
                error = tf.reduce_mean(
                    raw_error, name='error/cross_entropy_loss')
            elif type_of_loss == 'square_loss' or type_of_loss == 'rmse':
                raw_error = tf.squared_difference(
                    output, ratings, name='error/squared_diff'
                )
        ...
        with tf.control_dependencies(update_ops):
            ...
            if type_of_opt == 'adadelta':
                train_step = tf.train.AdadeltaOptimizer(self.eta).minimize(
                    loss, var_list=model_params)
            elif type_of_opt == 'sgd':
                train_step = tf.train.GradientDescentOptimizer(self.lr).minimize(
                    loss, var_list=model_params)
            elif type_of_opt == 'adam':
                train_step = tf.train.AdamOptimizer(self.lr).minimize(
                    loss, var_list=model_params)
            elif type_of_opt == 'ftrl':
                train_step = tf.train.FtrlOptimizer(self.lr).minimize(
                    loss, var_list=model_params)
            else:
                train_step = tf.train.GradientDescentOptimizer(self.lr).minimize(
                    loss, var_list=model_params)
